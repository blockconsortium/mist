...
class EthereumNode extends EventEmitter {
     ...
     * @param  {String} nodeType gexp, eth, etc
    ...
    _start(nodeType, network, syncMode) {
        ...
        return this.stop()
            ...
            .catch((err) => {
                ...
                // if unable to start eth node then write gexp to defaults
                if (nodeType === 'exp') {
                    ...
                    if (STATES.STARTING === this.state) {
                        ...
                        if (nodeType === 'gexp') {
                            if (dataStr.indexOf('fatal: error') >= 0) {
                                const error = new Error(`Gexp error: ${dataStr}`);
